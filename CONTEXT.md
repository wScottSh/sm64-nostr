...
- **Format descriptor** — the single machine-readable source of truth for the **packed payload** (~112–122 B in **format v2**; was 75 B in v1, #14), emitted by the same generator as the event profile. The ROM **pack adapter** and the host **unpack adapter** both derive from it, so the two sides can't drift. The packed payload is the system's narrowest, most permanent interface — the only artifact crossing ROM → reader → leaderboard.
