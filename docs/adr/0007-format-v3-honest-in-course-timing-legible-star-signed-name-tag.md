...
- `PIPELINE_EVENT_NAME` now enters the signed event and the wire; its validation
  ...
  (The ≤15 cap itself is later raised to ≤20 by spec #91, sub-issue #126,
  then shrunk back to ≤17 by spec #90 sub-issue #139 -- which supersedes
  #126; see `docs/format-v3-spec.md` for the current figure -- this ADR's
  numbers are a point-in-time historical record, not re-edited here.)
