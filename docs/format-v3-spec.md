...
- **tags** = `[["t","cabinet-leaderboard"],["t","sm64"],["n","<EVENT NAME>"]]`
  ...
  - `["n","<EVENT NAME>"]` — **new in v3**: the promoted `PIPELINE_EVENT_NAME`
    (charset A-Z/0-9/space, ≤17 chars as of spec #90 sub-issue #139 -- raised
    ≤15->≤20 by spec #91 sub-issue #126, then shrunk back ≤20->≤17 by #139,
    which SUPERSEDES #126; `gen_event_profile.py` normalization otherwise
    unchanged). Signed and on-wire; filterable (single-letter key `n`).
    Retires the "never reaches the wire" contract at
    `tools/gen_event_profile.py:42-51,82-83`.
