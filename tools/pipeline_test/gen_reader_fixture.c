 ...
 *
 * Every frame's URL text is recovered via qr_host_decode_alphanumeric() --
 * the SAME structural QR bitmap decoder main.c's own reassemble_built_event()
...
int main(void)
{
    ...
    printf("  \"_comment\": \"GENERATED by tools/pipeline_test/gen_reader_fixture.c -- DO NOT EDIT. "
           "A real build_event() call's actual emitted QR frames (URL text recovered via the same "
           "structural qr_host_decode_alphanumeric() main.c's own round-trip test uses), for the "
           "reader page's (reader/) host-side reassembly/decode/id-recompute test (spec #115, "
           "sub-issue #119). Regenerate: cd tools/pipeline_test && make reader-fixture\",\n");
    printf("  \"urlBase\": ");
    ...
    for (i = 0; i < event.frame_count; i++) {
        ...
        int urlTextLen = -1;
        if (!qr_host_decode_alphanumeric(event.qr_bitmaps[i], urlText, (int)sizeof(urlText), &urlTextLen)) {
            fprintf(stderr, "qr_host_decode_alphanumeric failed for frame %u\n", (unsigned)i);
            return 1;
        }
        printf("    ");
